...
**Warden** is the AI-Driven Development Lifecycle (AIDLC) security platform by [Techanv Consulting](https://warden.techanv.com). This component is an AI-powered security review GitHub Action that analyzes code changes for security vulnerabilities. It provides intelligent, context-aware security analysis for pull requests using Anthropic's Claude Code tool for deep semantic security analysis.

> Security as a blocking constraint, not a checklist.

> **Note:** Warden builds on Anthropic's open-source `claude-code-security-review` (MIT). It calls the Anthropic Claude API under the hood, so references to the `anthropic` SDK, the `ANTHROPIC_API_KEY` environment variable, and `claude-*` model IDs are functional and intentionally preserved.
...
1. Copy the [`security-review.md`](https://github.com/techanvconsulting/warden-action/blob/main/.claude/commands/security-review.md?plain=1) file from this repository to your project's `.claude/commands/` folder. 
